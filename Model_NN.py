...
def Model_NN(data, labels, test_data, test_target):
    ...
    out = np.zeros((test_data.shape[0], test_target.shape[1]))
    for i in range(len(test_data)):
        out[i, :] = simple_network.run(test_data[i,:])[:,0]

    Pred = np.round(out)
    Eval = evaluate_error(Pred.reshape(-1, 1), test_target)
